...
export default function Support() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Add some dummy tickets if API fails
        setTickets([
          {
            _id: "temp1",
            ...
            category: "billing",
            createdAt: new Date(),
            description: "Customer unable to process payment for event tickets"
          },
          {
            _id: "temp2", 
            subject: "Event Page Loading Error",
            status: "in_progress",
            priority: "medium", 
            category: "technical",
            createdAt: new Date(),
            description: "Event page showing 404 error for attendees"
          }
        ]);
      }
    } catch (error) {
  ...
  const handleCreateTicket = async (ticketData) => {
    try {
      ...
      if (response.ok) {
        ...
        // Create fallback ticket if API fails
        const newTicket = {
      ...
      // Create fallback ticket on error
      const newTicket = {
        _id: Date.now().toString(),
        ...ticketData, 
        status: 'open',
  ...
  const handleSendMessage = async (ticketId) => {
    ...
    try {
      const response = await authService.apiRequest(`/api/organizer/support/${ticketId}/messages`, {
        method: 'POST',
        body: JSON.stringify({
          message: newMessage,
          sender: 'organizer',
      ...
      if (response.ok) {
        ...
        // Create fallback message if API fails
        message = {
          _id: Date.now().toString(),
          message: newMessage,
          sender: 'organizer',
  ...
  const handleUpdateTicketStatus = async (ticketId, status) => {
    try {
      const response = await authService.apiRequest(`/api/organizer/support/${ticketId}`, {
        method: 'PUT',
                  ...
                  {filteredTickets.map((ticket) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500">
                        ...
                        <span className="whitespace-nowrap ml-2">{formatTicketDate(ticket.createdAt)}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="p-3 sm:p-4 lg:p-6 border-b border-gray-800">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 text-xs sm:text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white">{formatTicketDate(selectedTicket.createdAt)}</span>
                  </div>
                  <div className="flex items-center gap-2">
