...
export default function AdminUsersManagement() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await makeAdminApiCall("/api/admin/users");
  ...
  const handleViewUser = async (user) => {
    try {
      ...
      const response = await makeAdminApiCall(`/api/admin/users/${userId}`);
  ...
  const exportUsersData = () => {
    try {
      // Create comprehensive CSV data with all user information
      const csvHeaders = [
        'ID',
        'Full Name', 
        'Email',
      ...
      const csvData = filteredUsers.map(user => [
        ...
        (user.emailVerified && user.phoneVerified) ? 'Fully Verified' : 
        user.emailVerified ? 'Email Verified' : 
        user.phoneVerified ? 'Phone Verified' : 'Unverified'
      ]);

      // Combine headers and data
      const allRows = [csvHeaders, ...csvData];
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute("download", `users-export-${currentDate}.csv`);
        ...
        toast.success(`Users data exported successfully! (${filteredUsers.length} users)`);
      }
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center space-x-3">
          <button 
            ...
            title="Export all users data to CSV"
          >
                ...
                >
                  ...
                  <td className="p-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="relative">
                        <button 
                          onClick={() => handleStatusChange(user, (user.status || 'active') === 'active' ? 'suspended' : 'active')}
